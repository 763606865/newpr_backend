...
class UserCommerce extends ViewRecord
{
    protected static string $resource = UserResource::class;

    protected static ?string $title = '用户权益与订单';
    ...
    public function content(Schema $schema): Schema
    {
        return $schema->components([
            Section::make('用户信息')
                ->schema([
                    TextEntry::make('name')->label('真实姓名'),
                    TextEntry::make('nickname')->label('昵称')->placeholder('-'),
                    TextEntry::make('phone')->label('手机号')->placeholder('-'),
                ])
                ->columns(3),
            Section::make('权益余额')
                ->schema([
                    RepeatableEntry::make('asset_balances')
                        ->hiddenLabel()
                        ->getStateUsing(fn (User $record): array => AssetAccount::query()
                            ->where('owner_type', RcAssetOwnerType::User->value)
            ...
            Section::make('订单列表（最近 50 条）')
                ->schema([
                    RepeatableEntry::make('orders')
                        ->hiddenLabel()
                        ->getStateUsing(fn (User $record): array => Order::query()
                            ->where('payer_type', RcAssetOwnerType::User->value)
            ...
            Section::make('权益流水（最近 100 条）')
                ->schema([
                    RepeatableEntry::make('asset_ledgers')
                        ->hiddenLabel()
                        ->getStateUsing(fn (User $record): array => AssetLedger::query()
                            ->where('owner_type', RcAssetOwnerType::User->value)
                            ->where('owner_id', $record->id)
                            ->latest('id')
                            ->limit(100)
    ...
    private function increaseAssetAction(): Action
    {
        return Action::make('increaseAsset')
            ...
            ->color('success')
            ->authorize(fn (): bool => UserResource::canEdit($this->getRecord()))
            ->schema([
    ...
    private function addProductAction(): Action
    {
        return Action::make('addProduct')
            ...
            ->schema([
                Select::make('biz_plan_id')
                    ...
                    ->options(fn (): array => BizPlan::query()
                        ->where('target_side', RcBizPlanTargetSide::JobSeeker->value)
            ...
            ->action(function (array $data): void {
                UserRcCommerceService::make()->addProduct(
                    $this->getRecord(),
