...
| `candidate_external_user_id` | string\|null | 求职者 IM 外部用户 ID，用于招聘方发起沟通 |
...
```http
GET /rc/companies/applications?job_id=12&status=1&per_page=15
Authorization: Bearer {token}
```
...
- 接口：`POST /rc/companies/applications/{id}/invite-interview`
...
```http
POST /rc/companies/applications/18/invite-interview
Authorization: Bearer {token}
Content-Type: application/json
...
- 接口：`POST /rc/companies/applications/{id}/send-offer`
...
```http
POST /rc/companies/applications/18/send-offer
Authorization: Bearer {token}
Content-Type: application/json
...
- 接口：`POST /rc/companies/applications/{id}/hire`
...
```http
POST /rc/companies/applications/18/hire
Authorization: Bearer {token}
Content-Type: application/json
...
- 接口：`POST /rc/companies/applications/{id}/reject`
...
```http
POST /rc/companies/applications/18/reject
Authorization: Bearer {token}
Content-Type: application/json
...
1. 查看某职位收到的简历 `GET /rc/companies/applications?job_id={id}`
2. 查看候选人投递详情 `GET /rc/applications/{id}`（待处理自动变为筛选中）
3. 邀请面试 `POST /rc/companies/applications/{id}/invite-interview`（等待求职者确认）
4. 求职者接受后发送 Offer `POST /rc/companies/applications/{id}/send-offer`
5. 求职者接受 Offer `POST /rc/applications/{id}/accept-offer`
6. 确认录用 `POST /rc/companies/applications/{id}/hire`，或淘汰 `POST /rc/companies/applications/{id}/reject`
